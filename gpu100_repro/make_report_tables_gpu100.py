"""Assemble markdown tables + copy figures for gpu100_repro/REPORT.md.

Self-contained copy of scripts/make_report_tables.py pointed at
gpu100_repro/runs -> gpu100_repro/report_assets, so this round never touches
the original runs/ or report_assets/ from the CPU-only rounds.

    /tmp/gpu_bench/.venv/Scripts/python.exe gpu100_repro/make_report_tables_gpu100.py
"""
...
def sweep_tables(path: str, tag: str) -> str:
    ...
    out = [f"### {tag} - `{r['constellation']}`, "
           f"{r['episodes_per_point']} ep/point x {seeds} seed(s), "
...
def main() -> None:
    ...
    parts = ["# Auto-generated report assets (GPU / 100-agent round)\n"]
    ...
    eval_dirs = sorted(d for d in os.listdir(RUNS)
                       if d.startswith("g100_eval_") or d == "g100_reward_ablation"
                       and os.path.exists(os.path.join(RUNS, d, "results.json")))
    eval_dirs = [d for d in eval_dirs
                if os.path.exists(os.path.join(RUNS, d, "results.json"))]
    ...
    for extra in ("analysis", "g100_sensitivity"):
    ...
    with open(os.path.join(HERE, "report_assets", "TABLES.md"), "w", encoding="utf-8") as fh:
        fh.write("\n".join(parts))
    print("wrote gpu100_repro/report_assets/TABLES.md and figures")

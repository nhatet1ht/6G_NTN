...
### A4. Chưa dò `F` (hệ số phasing Walker) để khớp đúng đỉnh `N_max=27`

`ilcho-sanity` luôn ra đỉnh khả kiến Phase 2-a = **29**, không phải 27 như bài báo, vì `F=1`
là giả định (bài không công bố). Về nguyên tắc có thể **quét `F` qua vài giá trị nguyên** và
chọn giá trị cho đỉnh đúng 27 — chưa làm vì ảnh hưởng đã được xác nhận không đáng kể (đã kiểm
tra bằng khảo sát độ nhạy, hạng của các phương pháp không đổi), nên độ ưu tiên thấp.
...
bài báo", và lặp lại nhất quán nên nhiều khả năng phản ánh khác biệt thật về cách cài đặt
baseline hoặc tham số ẩn, không phải lỗi hay thiếu sót của bản tái hiện.
...
| A4 | Dò `F` cho đúng đỉnh N_max=27 | A | vài giờ, ưu tiên thấp |
...
| C1–C3 | 3 tuyên bố/số liệu không khớp | C | đã kết luận, không cần làm thêm |
